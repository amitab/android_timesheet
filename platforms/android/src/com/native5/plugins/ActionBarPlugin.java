...
public class ActionBarPlugin extends CordovaPlugin
{
	...
	ActionBar.OnNavigationListener navigation_listener = new ActionBar.OnNavigationListener()
	{
		...
		{
			webView.sendJavascript("var item = window.plugins.navBar.navigation_items[" + itemPosition + "]; if(item.click) item.click();");
			return true;
		}
	};
	...
	private boolean buildTabs(ActionBar bar, JSONArray definition, String menu_var)
	{
		try
		{
			for(int i = 0; i < definition.length(); ++i)
			{
				...
				final boolean isSelected = item_def.optBoolean("selected", false);
				
				bar.addTab(
						bar.newTab().setText(text).setContentDescription(text).setTag(text).setIcon(icon).setTabListener(new TabListener(this, menu_var + "[" + i + "]"))
						, isSelected);
			}
		}
		catch (JSONException e)
	...
	private final static List<String> plugin_actions = Arrays.asList(new String[] {
		...
		"setNavigationMode", "getNavigationMode", "setSelectedNavigationItem", "setSelectedTab","getSelectedNavigationItem",
